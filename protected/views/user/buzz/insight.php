<?php
    ...
    class Sentiment{
        ...
        public function categorise($sentance, $table)
        {
            //Access these text files to get the dictionary for each category
            $this->setDictionary('neg', $table);
            $this->setDictionary('pos', $table);
            $this->setDictionary('neu', $table);
			...
			if($classification == 'pos') {
			   $result = '<span class="green">Positive</span><br /><img src="'.Yii::app()->request->baseUrl.'/assets/buzz/positive.png"';
			}
			if($classification == 'neg') {
			   $result = '<span class="red">Negative</span><br /><img src="'.Yii::app()->request->baseUrl.'/assets/buzz/negative.png"';
			}
			if($classification == 'neu') {
			   $result = '<span class="grey">Neutral</span><br /><img src="'.Yii::app()->request->baseUrl.'/assets/buzz/neutral.png"';
			}
        ...
        public function setDictionary($class, $table) {
            
			$connection=Yii::app()->db;
			
            //Query to select the words for a type of classification
            $query = "SELECT word FROM $table WHERE type = '$class'";
            //Run Query
